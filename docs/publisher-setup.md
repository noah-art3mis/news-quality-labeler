...
Unpublished reviews are temporary: they expire after fifteen minutes, disappear on restart, and only the most recent hundred are retained. Published decisions and evidence remain in the database. Render deployments support [automatic ingestion](automatic-labeling.md). Automatic reclassification on dataset updates and subscriber-display monitoring remain outside the current scope.
