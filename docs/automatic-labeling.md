...
Create events with a matching source, a supported shortener, or a quote enter a durable queue. Updates and deletions also enter the queue so earlier labels can be withdrawn. Queued revisions of the same post coalesce to the newest event. A post revision receives its category set once. A changed revision replaces the previous labels, and deletion retracts the latest set. A manual retraction owns its revision: replay cannot republish that same revision.

The publication service remains the single writer for manual and automatic decisions. It saves an operation before delivery, finishes pending delivery before another decision for that post, and retries interrupted automatic work. Label history and evidence remain durable in `labels.db`. The operator page shows the latest decisions, with a bounded history view; older decisions remain in storage.
