...
export function createPreviewServer(preview: (input: string) => Promise<Preview>) {
  ...
  return createServer(async (request, response) => {
    ...
    if (request.method === 'GET' && request.url === '/') return send(200, renderPage());
    ...
    if (request.method !== 'POST' || request.url !== '/assess') return send(404, 'Not found.');
    ...
    try {
      ...
      input = new URLSearchParams(Buffer.concat(chunks).toString()).get('postUrl') ?? '';
      if (input.length > 2048) throw new Error('The submitted URL is too long.');
      const result = await preview(input);
      send(200, renderPage({ result, input }));
    } catch (error) {
      send(400, renderPage({ error: error instanceof Error ? error.message : 'Could not inspect this post.', input }));
    } finally { active--; }
  });
}
