...
export function openAutomaticStore(path: string, capacity = 10_000): AutomaticStore {
  ...
  const size = () => Number(db.prepare('SELECT count(*) AS n FROM automatic_jobs').get()!.n);
  return { cursor, size,
    accept(seq, job) {
      ...
      try {
        if (job) {
          if (!db.prepare('SELECT 1 FROM automatic_jobs WHERE uri=?').get(job.uri) && size() >= capacity) {
    ...
    complete(job) { db.prepare('DELETE FROM automatic_jobs WHERE uri=? AND seq=?').run(job.uri, job.seq); },
    close: () => db.close(),
  };
}
